...
// 查询{{.upperStartCamelObject}}记录
func (m *default{{.upperStartCamelObject}}Model) FindOne(ctx context.Context, {{.lowerStartCamelPrimaryKey}} {{.dataType}}) (out *{{.upperStartCamelObject}}, err error) {
	db := m.db.WithContext(ctx).Table(m.table)

	// 如果有条件语句
	err = db.Where("{{.originalPrimaryKey}} = ?", {{.lowerStartCamelPrimaryKey}}).First(&out).Error
	if err != nil {
		return nil, err
	}
